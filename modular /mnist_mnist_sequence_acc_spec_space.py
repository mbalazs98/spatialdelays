from mnist import download_and_parse_mnist_file
import numpy as np
from collections import namedtuple

from ml_genn import Connection, Network, Population
from ml_genn.callbacks import SpikeRecorder, Callback, VarRecorder
from ml_genn.compilers import EventPropCompiler
from ml_genn.connectivity import Dense, FixedProbability
from ml_genn.initializers import Normal, Uniform
from ml_genn.neurons import LeakyIntegrate, LeakyIntegrateFire, SpikeInput
from ml_genn.optimisers import Adam
from ml_genn.serialisers import Numpy
from ml_genn.synapses import Exponential


from ml_genn.compilers.event_prop_compiler import default_params
from ml_genn.utils.data import preprocess_tonic_spikes, linear_latency_encode_data
from argparse import ArgumentParser

import os
import shutil
from scipy.spatial.distance import cdist

import json


PreprocessedSpikes = namedtuple("PreprocessedSpikes", ["end_spikes", "spike_times"])

parser = ArgumentParser()
parser.add_argument("--num_hidden", type=int, default=64, help="Number of hidden neurons for MNIST")
parser.add_argument("--sparsity", type=float, default=0.01, help="Sparsity of connections")
parser.add_argument("--delays_within", type=int, default=1, help="Have delays within the module")
parser.add_argument("--k_reg", type=float, default=5e-11, help="Firing regularisation strength for mnist")
parser.add_argument("--noise", type=float, default=0, help="Amount of noise to add to the time difference")
parser.add_argument("--together", type=int, default=1, help="Initialise modules at same location or separately")
parser.add_argument("--seed", type=int, default=0, help="Random seed")
args = parser.parse_args()
np.random.seed(args.seed)
together = bool(args.together)

unique_suffix = "_".join(("_".join(str(i) for i in val) if isinstance(val, list) 
                         else str(val))
                         for arg, val in vars(args).items() if not arg.startswith("__"))

BATCH_SIZE = 256 
NUM_INPUT = 784
NUM_HIDDEN = args.num_hidden
#NUM_OUTPUT = 10
NUM_OUTPUT = 20


class EaseInSchedule(Callback):
    def __init__(self):
        pass
    def set_params(self, compiled_network, **kwargs):
        self._optimiser = compiled_network.optimisers[0][0]
    def on_batch_begin(self, batch):
        # Set parameter to return value of function
        if self._optimiser.alpha < 0.001 :
            self._optimiser.alpha = (self._optimiser.alpha) * (1.05 ** batch)
        else:
            self._optimiser.alpha = 0.001

labels_mnist_test = download_and_parse_mnist_file("t10k-labels-idx1-ubyte.gz", target_dir="../data")
mnist_test_images = download_and_parse_mnist_file("t10k-images-idx3-ubyte.gz", target_dir="../data")
labels_mnist_train = download_and_parse_mnist_file("train-labels-idx1-ubyte.gz", target_dir="../data")
mnist_train_images = download_and_parse_mnist_file("train-images-idx3-ubyte.gz", target_dir="../data")

def linear_latency_encode(data: np.ndarray, max_time: float,
                               min_time: float = 0.0,
                               thresh: int = 1):
    time_range = max_time - min_time
    # Get boolean mask of spiking neurons
    spike_vector = data > thresh

    # Take cumulative sum to get end spikes
    end_spikes = np.cumsum(spike_vector)

    # Extract values of spiking pixels
    spike_pixels = data[spike_vector]

    # Calculate spike times
    spike_times = (((255.0 - spike_pixels) / 255.0) * time_range) + min_time

    # Add to list
    return PreprocessedSpikes(end_spikes, spike_times)


EXAMPLE_TIME = 20.0
DT = 1.0
spikes_mnist_train, spikes_mnist_test =  mnist_train_images, mnist_test_images



def merge_paired_spikes(spikes, labels_orig):
    # Determine the number of pairs based on the smaller dataset
    num_pairs = len(spikes)
    
    # Randomly sample indices for both datasets
    indices_1 = np.random.choice(len(spikes), num_pairs, replace=False)
    indices_2 = np.random.choice(len(spikes), num_pairs, replace=False)
    sequence = np.random.randint(2, size=num_pairs)
    sequenced_spikes_1, sequenced_spikes_2, labels = [], [], []
    for ind_1, ind_2, seq in zip(indices_1, indices_2, sequence):
        sigma = (labels_orig[ind_1] + labels_orig[ind_2]) % 2
        noise = np.random.normal(100.0, args.noise,1)[0]
        if seq == 1:
            sequenced_spikes_1.append(linear_latency_encode(
                spikes[ind_1],
                EXAMPLE_TIME - (2.0 * DT), 2.0 * DT))
            sequenced_spikes_2.append(linear_latency_encode(
                spikes[ind_2],
                EXAMPLE_TIME + noise - (2.0 * DT), (2.0 + noise) * DT))
            labels.append(labels_orig[ind_1] * sigma + (1 - sigma) * (labels_orig[ind_2] + 10))
        else:
            sequenced_spikes_2.append(linear_latency_encode(
                spikes[ind_2],
                EXAMPLE_TIME - (2.0 * DT), 2.0 * DT))
            sequenced_spikes_1.append(linear_latency_encode(
                spikes[ind_1],
                EXAMPLE_TIME + noise - (2.0 * DT), (2.0 + noise) * DT))
            sigma = (labels_orig[ind_1] + labels_orig[ind_2]) % 2
            labels.append((labels_orig[ind_2] + 10) * sigma + (1 - sigma) * labels_orig[ind_1])
    return sequenced_spikes_1, sequenced_spikes_2, labels



network = Network(default_params)
with network:
    # Populations
    input_1 = Population(SpikeInput(max_spikes= BATCH_SIZE * 784),
                       NUM_INPUT)
    input_2 = Population(SpikeInput(max_spikes= BATCH_SIZE * 784),
                       NUM_INPUT)
    hidden_1 = Population(LeakyIntegrateFire(v_thresh=1.0, tau_mem=20.0,
                                           tau_refrac=None),
                        NUM_HIDDEN, record_spikes=True)
    

    hidden_2 = Population(LeakyIntegrateFire(v_thresh=1.0, tau_mem=20.0,
                                           tau_refrac=None),
                        NUM_HIDDEN, record_spikes=True)
    if together:
        loc1, loc2 = 30, -30
    else:
        loc1, loc2 = 0, 0
    x_pos1 = np.random.normal(loc=loc1, scale=1.0, size=NUM_HIDDEN)
    y_pos1 = np.random.normal(loc=loc1, scale=1.0, size=NUM_HIDDEN)
    z_pos1 = np.random.normal(loc=loc1, scale=1.0, size=NUM_HIDDEN)

    pos_dic = {}
    pos_dic[hidden_1] = (x_pos1, y_pos1, z_pos1)

    points1 = np.column_stack((x_pos1, y_pos1, z_pos1))
    hid1_hid1dist = cdist(points1, points1)

    x_pos2 = np.random.normal(loc=loc2, scale=1.0, size=NUM_HIDDEN)
    y_pos2 = np.random.normal(loc=loc2, scale=1.0, size=NUM_HIDDEN)
    z_pos2 = np.random.normal(loc=loc2, scale=1.0, size=NUM_HIDDEN)

    pos_dic[hidden_2] = (x_pos2, y_pos2, z_pos2)

    points2 = np.column_stack((x_pos2, y_pos2, z_pos2))
    hid2_hid2dist = cdist(points2, points2)

    hid1_hid2dist = cdist(points1, points2)
    hid2_hid1dist = cdist(points2, points1)

    output = Population(LeakyIntegrate(tau_mem=20.0, readout="max_var"),
                        NUM_OUTPUT)

    # Connections
    input_hidden_1 = Connection(input_1, hidden_1, Dense(Normal(mean=0.078, sd=0.045)),
               Exponential(5.0))
    
    input_hidden_2 = Connection(input_2, hidden_2, Dense(Normal(mean=0.078, sd=0.045)),
               Exponential(5.0))

    
    hidden_1_hidden_1 = Connection(hidden_1, hidden_1, Dense(Normal(mean=0.0, sd=0.02), delay=hid1_hid1dist),
               Exponential(5.0), max_delay_steps=255)
    hidden_2_hidden_2 = Connection(hidden_2, hidden_2, Dense(Normal(mean=0.0, sd=0.02), delay=hid2_hid2dist),
               Exponential(5.0), max_delay_steps=255)
    if args.sparsity == 1.0:
        hidden_1_hidden_2 = Connection(hidden_1, hidden_2, Dense(Normal(mean=0.0, sd=0.02), delay=hid1_hid2dist),
                Exponential(5.0), max_delay_steps=255)
        hidden_2_hidden_1 = Connection(hidden_2, hidden_1, Dense(Normal(mean=0.0, sd=0.02), delay=hid2_hid1dist),
                Exponential(5.0), max_delay_steps=255)
    elif args.sparsity > 0.0:
        hidden_1_hidden_2 = Connection(hidden_1, hidden_2, FixedProbability(p=args.sparsity, weight=Normal(mean=0.0, sd=0.02)),
                Exponential(5.0), max_delay_steps=255)
        all_pairs = [(i, j) for i in range(NUM_HIDDEN) for j in range(NUM_HIDDEN)]
        sampled_pairs = [all_pairs[idx] for idx in np.random.choice(len(all_pairs), size=max(1,int(NUM_HIDDEN * NUM_HIDDEN * args.sparsity)), replace=False)]
        hidden_1_hidden_2.connectivity.pre_ind = np.array([pair[0] for pair in sampled_pairs], dtype=int)
        hidden_1_hidden_2.connectivity.post_ind = np.array([pair[1] for pair in sampled_pairs], dtype=int)
        hidden_1_hidden_2.connectivity.weight = np.random.normal(loc=0.0, scale=0.02, size=max(1,int(NUM_HIDDEN * NUM_HIDDEN * args.sparsity)))
        hidden_1_hidden_2.connectivity.delay = hid1_hid2dist[hidden_1_hidden_2.connectivity.pre_ind, hidden_1_hidden_2.connectivity.post_ind].flatten()
        
        hidden_2_hidden_1 = Connection(hidden_2, hidden_1, FixedProbability(p=args.sparsity, weight=Normal(mean=0.0, sd=0.02)),
                Exponential(5.0), max_delay_steps=255)
        all_pairs = [(i, j) for i in range(NUM_HIDDEN) for j in range(NUM_HIDDEN)]
        sampled_pairs = [all_pairs[idx] for idx in np.random.choice(len(all_pairs), size=max(1,int(NUM_HIDDEN * NUM_HIDDEN * args.sparsity)), replace=False)]
        hidden_2_hidden_1.connectivity.pre_ind = np.array([pair[0] for pair in sampled_pairs], dtype=int)
        hidden_2_hidden_1.connectivity.post_ind = np.array([pair[1] for pair in sampled_pairs], dtype=int)
        hidden_2_hidden_1.connectivity.weight = np.random.normal(loc=0.0, scale=0.02, size=max(1,int(NUM_HIDDEN * NUM_HIDDEN * args.sparsity)))
        hidden_2_hidden_1.connectivity.delay = hid2_hid1dist[hidden_2_hidden_1.connectivity.pre_ind, hidden_2_hidden_1.connectivity.post_ind].flatten()

    hidden_1_output = Connection(hidden_1, output, FixedProbability(p=0.5, weight=Normal(mean=0.2, sd=0.37)),
               Exponential(5.0))
    pre_ind_1_out, post_ind_1_out = np.meshgrid(np.arange(NUM_HIDDEN), np.arange(10))

    # Flatten the arrays to get 1D arrays of all pairs
    pre_ind_1_out = pre_ind_1_out.flatten() 
    post_ind_1_out = post_ind_1_out.flatten()  
    hidden_1_output.connectivity.pre_ind = pre_ind_1_out
    hidden_1_output.connectivity.post_ind = post_ind_1_out
    #need to zero out connections from other module
    
    hidden_2_output = Connection(hidden_2, output, FixedProbability(p=0.5, weight=Normal(mean=0.2, sd=0.37)),
               Exponential(5.0))

    pre_ind_2_out, post_ind_2_out = np.meshgrid(np.arange(NUM_HIDDEN), np.arange(10,20))
    pre_ind_2_out = pre_ind_2_out.flatten()
    post_ind_2_out = post_ind_2_out.flatten()
    hidden_2_output.connectivity.pre_ind = pre_ind_2_out
    hidden_2_output.connectivity.post_ind = post_ind_2_out
    

k_reg = {}

k_reg[hidden_1] = args.k_reg
k_reg[hidden_2] = args.k_reg
delay_learn_conns = [hidden_1_hidden_2,hidden_2_hidden_1, hidden_1_hidden_1, hidden_2_hidden_2]
pops_and_conns=([hidden_1, hidden_2],[[hidden_1_hidden_1, hidden_1_hidden_2],[hidden_2_hidden_2, hidden_2_hidden_1] ],[[hidden_1_hidden_1, hidden_1_hidden_2],[hidden_2_hidden_2, hidden_1_hidden_2]])
conns_and_pops=([hidden_1_hidden_1, hidden_2_hidden_2, hidden_1_hidden_2, hidden_2_hidden_1],[hidden_1, hidden_2, hidden_1, hidden_2],[hidden_1,hidden_2, hidden_2, hidden_1])

max_example_timesteps = int(np.ceil(EXAMPLE_TIME / DT)) +  100
try:
    shutil.rmtree("checkpoints_mnist_sequence_space_" + unique_suffix)
except FileNotFoundError:
    pass
serialiser = Numpy("checkpoints_mnist_sequence_space_" + unique_suffix)
'''compiler = EventPropCompiler(example_timesteps=max_example_timesteps,
                                losses="sparse_categorical_crossentropy",
                                reg_lambda_upper=k_reg, reg_lambda_lower=0, 
                                reg_nu_upper=1, max_spikes=1500,
                                delay_learn_conns=delay_learn_conns,
                                optimiser=Adam(0.001 * 0.01), delay_optimiser=Adam(1.0),
                                batch_size=BATCH_SIZE, rng_seed=args.seed)'''

compiler = EventPropCompiler(example_timesteps=max_example_timesteps,
                                losses="sparse_categorical_crossentropy",
                                reg_lambda_upper=k_reg, reg_lambda_lower=k_reg, 
                                reg_nu_upper=1, max_spikes=1500, delay_learn_conns=delay_learn_conns,
                                pops_and_conns=pops_and_conns,
                                conns_and_pops=conns_and_pops,
                                optimiser=Adam(0.001), x_optimiser=Adam(0.01), y_optimiser=Adam(0.01), z_optimiser=Adam(0.01),
                                pos_init = pos_dic,
                                batch_size=BATCH_SIZE, rng_seed=args.seed)

model_name = (f"classifier_train_{md5(unique_suffix.encode()).hexdigest()}"
                  if os.name == "nt" else f"classifier_train_{unique_suffix}")
compiled_net = compiler.compile(network, name=model_name)

# Apply augmentation to events and preprocess

spikes_train_1, spikes_train_2, labels_train = merge_paired_spikes(spikes_mnist_train, labels_mnist_train)
spikes_test_1, spikes_test_2, labels_test = merge_paired_spikes(spikes_mnist_test, labels_mnist_test)
results_dic = {}

with compiled_net:
    # Loop through epochs
    callbacks = [SpikeRecorder(hidden_1, key="hidden_1_spikes", record_counts=True), SpikeRecorder(hidden_2, key="hidden_2_spikes", record_counts=True), EaseInSchedule()]
    validation_callbacks = []
    best_acc = 0
    early_stop = 15

    for e in range(500):
        
        # Train epoch
        train_metrics, valid_metrics, train_cb, valid_cb  = compiled_net.train({input_1: spikes_train_1, input_2: spikes_train_2},
                                            {output: labels_train},
                                            start_epoch=e, num_epochs=1, 
                                            shuffle=True, callbacks=callbacks, validation_callbacks=validation_callbacks, validation_x={input_1: spikes_test_1, input_2: spikes_test_2}, validation_y={output: labels_test})

        
        
        hidden_1_spikes = np.zeros(NUM_HIDDEN)
        for cb_d in train_cb['hidden_1_spikes']:
            hidden_1_spikes += cb_d
        

        _input_hidden_1 = compiled_net.connection_populations[input_hidden_1]
        _input_hidden_1.vars["g"].pull_from_device()
        g_view = _input_hidden_1.vars["g"].view.reshape((784, NUM_HIDDEN))
        g_view[:,hidden_1_spikes==0] += 0.002
        _input_hidden_1.vars["g"].push_to_device()
        
        hidden_2_spikes = np.zeros(NUM_HIDDEN)
        for cb_d in train_cb['hidden_2_spikes']:
            hidden_2_spikes += cb_d

        _input_hidden_2 = compiled_net.connection_populations[input_hidden_2]
        _input_hidden_2.vars["g"].pull_from_device()
        g_view = _input_hidden_2.vars["g"].view.reshape((784, NUM_HIDDEN))
        g_view[:,hidden_2_spikes==0] += 0.002
        _input_hidden_2.vars["g"].push_to_device()


        
        
        if train_metrics[output].result > best_acc:
            best_acc = train_metrics[output].result
            early_stop = 15
            results_dic["accuracy"] = valid_metrics[output].result
            compiled_net.save(("best",), serialiser)
            compiled_net.save_connectivity(("best",), serialiser)
            Pop1 = compiled_net.neuron_populations[hidden_1]
            np.save("checkpoints_mnist_sequence_space_" + unique_suffix + "/XPos1.npy", Pop1.vars["XPos"].values)
            np.save("checkpoints_mnist_sequence_space_" + unique_suffix + "/YPos1.npy", Pop1.vars["YPos"].values)
            np.save("checkpoints_mnist_sequence_space_" + unique_suffix + "/ZPos1.npy", Pop1.vars["ZPos"].values)
            Pop2 = compiled_net.neuron_populations[hidden_2]
            np.save("checkpoints_mnist_sequence_space_" + unique_suffix + "/XPos2.npy", Pop2.vars["XPos"].values)
            np.save("checkpoints_mnist_sequence_space_" + unique_suffix + "/YPos2.npy", Pop2.vars["YPos"].values)
            np.save("checkpoints_mnist_sequence_space_" + unique_suffix + "/ZPos2.npy", Pop2.vars["ZPos"].values)
        else:
            early_stop -= 1
            if early_stop < 0:
                break
        
    


network_weights = "checkpoints_mnist_sequence_space_" + unique_suffix + "/"

with network:
    # Populations
    input_1 = Population(SpikeInput(max_spikes= BATCH_SIZE * 784),
                       NUM_INPUT)
    input_2 = Population(SpikeInput(max_spikes= BATCH_SIZE * 784),
                       NUM_INPUT)
    hidden_1 = Population(LeakyIntegrateFire(v_thresh=1.0, tau_mem=20.0,
                                           tau_refrac=None),
                        NUM_HIDDEN, record_spikes=True)
    hidden_2 = Population(LeakyIntegrateFire(v_thresh=1.0, tau_mem=20.0,
                                           tau_refrac=None),
                        NUM_HIDDEN, record_spikes=True)
    output_1 = Population(LeakyIntegrate(tau_mem=20.0, readout="max_var"),
                        int(NUM_OUTPUT/2))
    output_2 = Population(LeakyIntegrate(tau_mem=20.0, readout="max_var"),
                        int(NUM_OUTPUT/2))

    output_3 = Population(LeakyIntegrate(tau_mem=20.0, readout="max_var"),
                        int(NUM_OUTPUT/2))
    output_4 = Population(LeakyIntegrate(tau_mem=20.0, readout="max_var"),
                        int(NUM_OUTPUT/2))

    # Connections
    input_hidden_1 = Connection(input_1, hidden_1, Dense(Normal(mean=0.078, sd=0.045)),
               Exponential(5.0))
    
    input_hidden_1.connectivity.weight = np.load(network_weights+"best-Conn_Pop0_Pop2-g.npy")

    
    input_hidden_2 = Connection(input_2, hidden_2, Dense(Normal(mean=0.078, sd=0.045)),
               Exponential(5.0))

    input_hidden_2.connectivity.weight = np.load(network_weights+"best-Conn_Pop1_Pop3-g.npy")

    
    hidden_1_hidden_1 = Connection(hidden_1, hidden_1, Dense(Normal(mean=0.0, sd=0.02), delay=Uniform(0.0, 0.0)),
               Exponential(5.0), max_delay_steps=255)
    hidden_1_hidden_1.connectivity.weight = np.load(network_weights+"best-Conn_Pop2_Pop2-g.npy")
    hidden_2_hidden_2 = Connection(hidden_2, hidden_2, Dense(Normal(mean=0.0, sd=0.02), delay=Uniform(0.0, 0.0)),
               Exponential(5.0), max_delay_steps=255)
    hidden_2_hidden_2.connectivity.weight = np.load(network_weights+"best-Conn_Pop3_Pop3-g.npy")
    if True:#args.delays_within:
        hidden_1_hidden_1.connectivity.delay = np.load(network_weights+"best-Conn_Pop2_Pop2-d.npy")
        hidden_2_hidden_2.connectivity.delay = np.load(network_weights+"best-Conn_Pop3_Pop3-d.npy")
    if args.sparsity == 1.0:
        hidden_1_hidden_2 = Connection(hidden_1, hidden_2, Dense(Normal(mean=0.0, sd=0.02), delay=Uniform(0.0, 0.0)),
                Exponential(5.0), max_delay_steps=255)
        hidden_2_hidden_1 = Connection(hidden_2, hidden_1, Dense(Normal(mean=0.0, sd=0.02), delay=Uniform(0.0,0.0)),
                Exponential(5.0), max_delay_steps=255)
    elif args.sparsity > 0.0:
        connectivity_hidden_1_hidden_2 = FixedProbability(p=args.sparsity, weight=Normal(mean=0.0, sd=0.02), delay=Uniform(0.0,0.0))
        connectivity_hidden_1_hidden_2.pre_ind = np.load(network_weights+"best-Conn_Pop2_Pop3-pre_ind.npy")
        connectivity_hidden_1_hidden_2.post_ind = np.load(network_weights+"best-Conn_Pop2_Pop3-post_ind.npy")
        hidden_1_hidden_2 = Connection(hidden_1, hidden_2, connectivity_hidden_1_hidden_2,
                Exponential(5.0), max_delay_steps=255)
        
        connectivity_hidden_2_hidden_1 = FixedProbability(p=args.sparsity, weight=Normal(mean=0.0, sd=0.02), delay=Uniform(0.0,0.0))
        connectivity_hidden_2_hidden_1.pre_ind = np.load(network_weights+"best-Conn_Pop3_Pop2-pre_ind.npy")
        connectivity_hidden_2_hidden_1.post_ind = np.load(network_weights+"best-Conn_Pop3_Pop2-post_ind.npy")
        hidden_2_hidden_1 = Connection(hidden_2, hidden_1, connectivity_hidden_2_hidden_1,
                Exponential(5.0), max_delay_steps=255)
    
    hidden_1_hidden_2.connectivity.weight = np.load(network_weights+"best-Conn_Pop2_Pop3-g.npy")
    hidden_1_hidden_2.connectivity.delay = np.load(network_weights+"best-Conn_Pop2_Pop3-d.npy")
    hidden_2_hidden_1.connectivity.weight = np.load(network_weights+"best-Conn_Pop3_Pop2-g.npy")
    hidden_2_hidden_1.connectivity.delay = np.load(network_weights+"best-Conn_Pop3_Pop2-d.npy")

    hidden_1_output1 = Connection(hidden_1, output_1, Dense(weight=Normal(mean=0.2, sd=0.37)),
               Exponential(5.0))
    hidden_1_output2 = Connection(hidden_1, output_2, Dense(weight=Normal(mean=0.2, sd=0.37)),
               Exponential(5.0))
    
    hidden_2_output3 = Connection(hidden_2, output_3, Dense(weight=Normal(mean=0.2, sd=0.37)),
               Exponential(5.0))
    hidden_2_output4 = Connection(hidden_2, output_4, Dense(weight=Normal(mean=0.2, sd=0.37)),
               Exponential(5.0))
    
k_reg = {}

k_reg[hidden_1] = args.k_reg
k_reg[hidden_2] = args.k_reg

max_example_timesteps = int(np.ceil(EXAMPLE_TIME / DT)) +  100
compiler = EventPropCompiler(example_timesteps=max_example_timesteps,
                                losses="sparse_categorical_crossentropy",
                                reg_lambda_upper=k_reg, reg_lambda_lower=0, 
                                reg_nu_upper=1, max_spikes=1500,
                                delay_learn_conns=[],
                                optimiser=Adam(0.001 * 0.01), delay_optimiser=Adam(0.0), weight_fix_conns=[input_hidden_1, input_hidden_2, hidden_1_hidden_1, hidden_2_hidden_2, hidden_1_hidden_2, hidden_2_hidden_1],
                                batch_size=BATCH_SIZE, rng_seed=args.seed)

model_name = (f"classifier_train_{md5(unique_suffix.encode()).hexdigest()}"
                  if os.name == "nt" else f"classifier_train_{unique_suffix}")
compiled_net = compiler.compile(network, name=model_name)

# Apply augmentation to events and preprocess

def merge_paired_spikes2(spikes, labels_orig):
    # Determine the number of pairs based on the smaller dataset
    num_pairs = len(spikes)
    
    # Randomly sample indices for both datasets
    indices_1 = np.random.choice(len(spikes), num_pairs, replace=False)
    indices_2 = np.random.choice(len(spikes), num_pairs, replace=False)
    sequence = np.random.randint(2, size=num_pairs)
    sequenced_spikes_1, sequenced_spikes_2, labels_1, labels_2 = [], [], [], []
    for ind_1, ind_2, seq in zip(indices_1, indices_2, sequence):
        if seq == 1:
            sequenced_spikes_1.append(linear_latency_encode(
                spikes[ind_1],
                EXAMPLE_TIME - (2.0 * DT), 2.0 * DT))
            sequenced_spikes_2.append(linear_latency_encode(
                spikes[ind_2],
                EXAMPLE_TIME + 100.0 - (2.0 * DT), (2.0 + 100.0) * DT))
        else:
            sequenced_spikes_2.append(linear_latency_encode(
                spikes[ind_2],
                EXAMPLE_TIME - (2.0 * DT), 2.0 * DT))
            sequenced_spikes_1.append(linear_latency_encode(
                spikes[ind_1],
                EXAMPLE_TIME + 100.0 - (2.0 * DT), (2.0 + 100.0) * DT))
        labels_1.append(labels_orig[ind_1])
        labels_2.append(labels_orig[ind_2])
    return sequenced_spikes_1, sequenced_spikes_2, labels_1, labels_2

spikes_train_1, spikes_train_2, labels_train_1, labels_train_2 = merge_paired_spikes2(spikes_mnist_train, labels_mnist_train)
spikes_test_1, spikes_test_2, labels_test_1, labels_test_2  = merge_paired_spikes2(spikes_mnist_test, labels_mnist_test)

with compiled_net:
    # Loop through epochs
    callbacks = [EaseInSchedule()]
    validation_callbacks = []
    best_acc_1, best_acc_2, best_acc_3, best_acc_4 = 0, 0, 0, 0
    early_stop = 15
    for e in range(500):
        
        # Train epoch
        train_metrics, valid_metrics, train_cb, valid_cb  = compiled_net.train({input_1: spikes_train_1, input_2: spikes_train_2},
                                            {output_1: labels_train_1, output_2: labels_train_2, output_3: labels_train_1, output_4: labels_train_2},
                                            start_epoch=e, num_epochs=1, 
                                            shuffle=True, callbacks=callbacks, validation_callbacks=validation_callbacks, validation_x={input_1: spikes_test_1, input_2: spikes_test_2}, validation_y={output_1: labels_test_1, output_2: labels_test_2, output_3: labels_test_1, output_4: labels_test_2})

        

        
        not_improved = True
        if train_metrics[output_1].result > best_acc_1:
            best_acc_1 = train_metrics[output_1].result
            results_dic["spec1_correct"] = valid_metrics[output_1].result
            results_dic["spec1_incorrect"] = valid_metrics[output_2].result
            #results_dic["spec1"] = valid_metrics[output_1].result - valid_metrics[output_2].result
            early_stop = 15
            not_improved = False
        if train_metrics[output_2].result > best_acc_2:
            best_acc_2 = train_metrics[output_2].result
            results_dic["spec1_correct"] = valid_metrics[output_1].result
            results_dic["spec1_incorrect"] = valid_metrics[output_2].result
            #results_dic["spec1"] = valid_metrics[output_1].result - valid_metrics[output_2].result
            early_stop = 15
            not_improved = False
        if train_metrics[output_3].result > best_acc_3:
            best_acc_3 = train_metrics[output_3].result
            results_dic["spec2_correct"] = valid_metrics[output_4].result
            results_dic["spec2_incorrect"] = valid_metrics[output_3].result
            #results_dic["spec2"] = valid_metrics[output_4].result - valid_metrics[output_3].result
            early_stop = 15
            not_improved = False
        if train_metrics[output_4].result > best_acc_4:
            best_acc_4 = train_metrics[output_4].result
            results_dic["spec2_correct"] = valid_metrics[output_4].result
            results_dic["spec2_incorrect"] = valid_metrics[output_3].result
            #results_dic["spec2"] = valid_metrics[output_4].result - valid_metrics[output_3].result
            early_stop = 15
            not_improved = False
        if not_improved:
            early_stop -= 1
            if early_stop < 0:
                break



with open(f"results_mnist_space/acc_spec_{unique_suffix}.json", 'w') as f:
    json.dump(results_dic, f, indent=4)
