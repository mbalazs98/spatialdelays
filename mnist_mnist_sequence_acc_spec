...
with network:
    ...
    if args.sparsity == 1.0:
        ...
        hidden_1_hidden_2 = Connection(hidden_1, hidden_2, FixedProbability(p=args.sparsity, weight=Normal(mean=0.0, sd=0.02), delay=Uniform(args.delay_init, args.delay_init)),
                Exponential(5.0), max_delay_steps=255)
        ...
        hidden_2_hidden_1 = Connection(hidden_2, hidden_1, FixedProbability(p=args.sparsity, weight=Normal(mean=0.0, sd=0.02), delay=Uniform(args.delay_init, args.delay_init)),
                Exponential(5.0), max_delay_steps=255)
...
with compiled_net:
    ...
    for e in range(500):
        ...
        if train_metrics[output].result > best_acc:
            ...
            for filename in os.listdir("checkpoints_mnist_sequence_" + unique_suffix):
                os.unlink(os.path.join("checkpoints_mnist_sequence_" + unique_suffix, filename))
...
with open(f"results/acc_spec_{unique_suffix}.json", 'w') as f:
